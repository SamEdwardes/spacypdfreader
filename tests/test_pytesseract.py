...
def test_pytesseract_multi_with_params():
    params = {"nice": 1}
    nlp = spacy.load("en_core_web_sm")
    doc = pdf_reader("tests/data/test_pdf_01.pdf", nlp, pytesseract.parser, n_processes=4, **params)
    pdf_assertions(doc)


def test_pytesseract_multi_same_as_single():
    nlp = spacy.load("en_core_web_sm")
    doc_multi = pdf_reader("tests/data/test_pdf_01.pdf", nlp, pytesseract.parser, n_processes=4)
